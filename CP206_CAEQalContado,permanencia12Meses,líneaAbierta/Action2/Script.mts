...
Sub CambioSimplificado()
 ...
 @@ hightlight id_;_29628800_;_script infofile_;_ZIP::ssf8.xml_;_
	If ((ucase(Motivo) = "CAEQ_EQUIPO") or (ucase(Motivo) = "CAEQ_CAPL")) Then
		...
		End If
		wait 1
		if (ucase(DataTable("e_Financiamiento", dtlocalSheet)) = "NO" and ucase(DataTable("e_FormaPago", dtlocalSheet)) = "SI") then
	...
	If ucase(Motivo) = "CAPL" Then
        ...
        Dim g
        g = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Cambio Simplificado (Para").JavaCheckBox("Cambiar forma de pago").GetROProperty("enabled")
		if (ucase(DataTable("e_Financiamiento", dtlocalSheet)) = "NO" and g = "1" and ucase(DataTable("e_FormaPago", dtlocalSheet)) = "SI") then
			JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Cambio Simplificado (Para").JavaCheckBox("Cambiar forma de pago").Set "ON"
		End If
	End If
	wait 1	
	JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "finExpress.png", True
	imagenToWord "Cambio Finalizado  ",RutaEvidencias() & "finExpress.png" 
...
Sub OrdenCerrada()
	...
	If ucase(DataTable("Motivo", dtLocalSheet)) = "CAPL" Then
		...
		While not (DataTable("s_ValEstadoOrden", dtLocalSheet) = "Cerrado" or DataTable("s_ValEstadoOrden", dtLocalSheet) = "Programado" or DataTable("s_ValEstadoOrden", dtLocalSheet) = "Enviado")
			wait 1
			DataTable("s_ValEstadoOrden", dtLocalSheet) = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Buscar: Orden").JavaTable("Ver por:").GetCellData("#0","#4")
		Wend
		If DataTable("s_ValEstadoOrden", dtLocalSheet) = "Cerrado" or DataTable("s_ValEstadoOrden", dtLocalSheet) = "Programado" Then
		...
		ElseIf DataTable("s_ValEstadoOrden", dtLocalSheet) = "Enviado" Then
			...
			wait 1
				...
				wait 8
				DataTable("s_ValEstadoOrden", dtLocalSheet) = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Buscar: Orden").JavaTable("Ver por:").GetCellData("#0","#4")
				While not (DataTable("s_ValEstadoOrden", dtLocalSheet) = "Cerrado" )
					wait 1
					JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Buscar: Orden").JavaButton("Buscar ahora").Click
					wait 8
					DataTable("s_ValEstadoOrden", dtLocalSheet) = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Buscar: Orden").JavaTable("Ver por:").GetCellData("#0","#4")
		...
		While not (DataTable("s_ValEstadoOrden", dtLocalSheet) = "Cerrado" )
			...
			DataTable("s_ValEstadoOrden", dtLocalSheet) = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Buscar: Orden").JavaTable("Ver por:").GetCellData("#0","#4")
		Wend
		If DataTable("s_ValEstadoOrden", dtLocalSheet) = "Cerrado" Then
			DataTable("s_Resultado", dtLocalSheet)="Exito"
			DataTable("s_Detalle", dtLocalSheet)="La orden finalizó correctamente"
			Reporter.ReportEvent micPass,DataTable("s_Resultado", dtLocalSheet),DataTable("s_Detalle", dtLocalSheet)
			wait 2
